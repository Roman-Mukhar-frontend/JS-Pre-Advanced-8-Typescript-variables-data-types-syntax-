...
// Число непарне.
// Число 0.
let numFromPrompt = prompt('Введіть число');
let number = numFromPrompt;
...
let cenzorBtn = document.querySelector('.big-btn');
let inputBadWords = document.querySelector('.input-one');
let inputText = document.querySelector('.input-two');
// FUNCTION BUTTON "ADD"
...
cenzorBtn.addEventListener('click', function () {
    if (inputText.value == '') {
        ...
        let badWordsList = badWords.textContent?.split(',');
        let textForCenzor = inputText.value;
        for (let i = 0; i < badWordsList.length; i++) {
            let badWord = badWordsList[i];
            while (textForCenzor.includes(badWord)) {
                let word = "*";
                for (let a = 1; a < badWord.length; a++) {
                    word += "*";
                }
